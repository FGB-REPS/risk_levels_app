...
When selecting data types, focus on what makes the data type unique from other types. In other words, if you select clinical report forms (CRFs), the raw version of these data often include other overlapping data types such as name, age, date of birth and gender. If you will be collecting CRF data will all of those other overlapping data types, then make sure to also select name, age, date of birth and gender in addition to CRF data. If you will be collecting CRF data that has been stripped of this overlapping information, then select only CRF data.

### Participant types

**NOTE**: The term "participant type" is primarily used in this app for legibility, however, you are primarily concerned here with the individuals the data are about, which is why you may also see the term "research subjects". The goal is to determine who the data are referring to, which in some cases can be multiple research subjects. For example, if your participants are parents answering questions about their children, the participant type depends on whether the data collected is *just* about the children (e.g. objective data on the children) or also includes data that could say something about the parents (e.g. subjective questions about their child's behaviour). It is also important to remember this concept when reusing existing data, since you may not have participants in your study, but the data are still about specific individuals who are the research subjects that the data are about.
...
**NB**: Some participant types will automatically adjust the input for the data sensitivity section from "No" to "Yes" because of how vulnerable these participants are. You can still select the appropriate sensitivity (see the next section for further information), i.e. even if the participants are very vulnerable, but the information is definitely not sensitive in nature, you can return the input selection to "No"

### Risk of harm

When contemplating risk of harm, consider whether the information or subject matter that your data represent are about sensitive, taboo, stigmatized or otherwise potentially harmful topics. If the data are [special under the GDPR](https://fgb-rdm.nl/legal/GDPRBasics.html#special){target="_blank"} the risk of harm *may* be elevated, but this is not always the case; use the "special" data category as a warning sign for elevated risks of harm. The primary rule of thumb is to contemplate whether the information represented in the data add additional risks of harm to the research participants, in combination with and/or beyond what is already known about the participants' vulnerability. More information on [the risk of harm concept can be found on this page](https://fgb-rdm.nl/Security/PrivacyRisks.html#harm){target="_blank"}.

The default for risk of harm is set to "No", but you can adjust this to "Yes" if the content of your data elevate the risks of harm. For certain participant populations that belong to highly stigmatized or marginalized groups, the risk of harm will automatically switch to a default of "Yes". In such a case, you can return the risk of harm to "No" if in your estimation the risks posed by the data to the research population are not elevated. You can also discuss this further with the [FGB Privacy Officer](mailto:research.data.fgb@vu.nl) if you are unsure.
